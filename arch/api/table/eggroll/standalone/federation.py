...
class FederationRuntime(object):
    ...
    def __init__(self, job_id, runtime_conf):
        self._eggroll_federation = init(job_id, runtime_conf)

    def _get_all(self, name, tag) -> list:
        rtn = self._eggroll_federation.get(name=name, tag=tag, idx=-1)
        if len(rtn) > 0 and isinstance(rtn[0], _DTable):
            rtn = [EggRollTable(v) for v in rtn]
        return rtn

    def _get_single(self, name, tag, idx) -> EggRollTable:
        rtn = self._eggroll_federation.get(name=name, tag=tag, idx=idx)
        if isinstance(rtn, _DTable):
            rtn = EggRollTable(rtn)
        return rtn

    def _idx_in_range(self, name, idx) -> bool:
        if idx < 0:
            return False
        algorithm, sub_name = self._eggroll_federation.__check_authorization(name, is_send=False)
        auth_dict = self._eggroll_federation.trans_conf.get(algorithm)
        src_role = auth_dict.get(sub_name).get('src')
        src_party_ids = self._eggroll_federation.__get_parties(src_role)
    ...
    def remote(self, obj, name: str, tag: str, role=None, idx=-1):
        if isinstance(obj, EggRollTable):
            return self._eggroll_federation.remote(obj=obj.get_dtable(), name=name, tag=tag, role=role, idx=idx)
        else:
